...
class Master extends CI_Controller
{
  ...
  public function datatable()
  {
    ...
    $table = 'tamu';
    $col_order = [null, 'nama', 'alamat', 'keterangan'];
    $order = ['id' => 'asc'];
    $search = ['nama', 'alamat', 'vip', 'keterangan'];
    ...
    foreach ($list as $key) {
      ...
      if ($key['vip'] == 'vip') {
        $vip = '<span style="font-size:11px;padding: 4px 4px" class="badge badge-pill badge-primary float-right">VIP</span>';
      } else {
        $vip = '';
      }
      ...
      $no++;
      $row = array();
      $row[] = $no . '<div class="d-inline-block float-right"><a href="" data-id="' . $key['id'] . '" data-nama="' . $key['nama'] . '" data-alamat="' . $key['alamat'] . '" data-keterangan="' . $key['keterangan'] . '" data-vip="' . $key['vip'] . '" class="btnEdit" data-toggle="tooltip" data-placement="top" title="Edit"><i class="mdi mdi-pencil"></i></a><a href="" data-id="' . $key['id'] . '" class="btnDelet ml-2 text-danger" data-toggle="tooltip" data-placement="top" title="Delete"><i class="mdi mdi-delete"></i></a></div>';
      $row[] = $key['nama'] . ' ' . $vip;
      $row[] = $key['alamat'];
      $row[] = $key['keterangan'];
      // $row[] = $key['telp'];
      $row[] = '<div class="btn-group btn-group-sm"><a href="" data-id="' . $key['id'] . '" class="btnLinkWp btn btn-secondary" data-toggle="tooltip" data-placement="top" title="Undangan WP"><i class="mdi mdi-wordpress"></i></a><a href="" data-id="' . $key['id'] . '" class="btn btn-success btnWA" data-toggle="tooltip" data-placement="top" title="Share WhatsApp"><i class="mdi mdi-whatsapp"></i></a><a href="" data-id="' . $key['id'] . '" class="btn btn-info btnCopy" data-toggle="tooltip" data-placement="top" title="Copy Pesan"><i class="mdi mdi-content-copy"></i></a><a href="' . base_url('master/qrDownload/' . $key['id']) . '" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title=" QRcode"><i class="mdi mdi-qrcode-scan"></i></a></div>';
  ...
  {
    $event = $this->session->userdata('sessionEventCek');
    $ev = $this->m_event->byId($event);
    $this->load->model('m_gridm');
    $table = 'tamu';

    $list = $this->m_gridm->getTamuByEvent($table, $event);
    $data = '';
    if ($list) {
      ...
      foreach ($list as $key) {
        ...
        if ($key['vip'] == 'vip') {
          $vip = '<span style="font-size:11px;padding: 4px 4px" class="badge badge-pill badge-primary">VIP</span>';
        } else {
          $vip = '';
        }

        if ($ev['sesi'] > 1) {
          $sesi = '<span style="font-size:9px;padding: 2px 8px" class="badge badge-pill badge-info">Sesi ' . $key['sesi'] . '</span>';
        } else {
          $sesi = '';
        }
        

        $data .= '<div class="item-box">';
        $data .= '<span class="urutan">' . $i . '</span> ';
        $data .= '<div class="nama">' . $key['nama'] . ' ' . $vip . '</div>';
        $data .= '<div class="alamat">' . $key['alamat'] . ' | ' . $key['keterangan'] . ' ' . $sesi . '</div>';
        $data .= '<input type="text" style="display:none" id="idT" value="' . $key['id'] . '">';
        $data .= '<div class="boxbutton">';
        $data .= '<div class="dropdown">';
        $data .= ' <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenuButton"
      data-toggle="dropdown" aria-expanded="false"><i class="mdi mdi-settings"></i></button>';
        $data .= '<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">';
        $data .= '<a class="dropdown-item btnLinkWp" data-id="' . $key['id'] . '" href="#">Link</a>';
        $data .= '<a class="dropdown-item btnWA" data-id="' . $key['id'] . '" href="#">WhatsApp</a>';
        $data .= '<a class="dropdown-item btnCopy" data-id="' . $key['id'] . '" href="#">Copy</a>';
        $data .= '<a class="dropdown-item" href="' . base_url('master/qrDownload/' . $key['id']) . '">Qrcode</a>';
        $data .= '<a class="dropdown-item btnEdit"  data-id="' . $key['id'] . '" data-nama="' . $key['nama'] . '" data-alamat="' . $key['alamat'] . '" data-keterangan="' . $key['keterangan'] . '" data-vip="' . $key['vip'] . '" data-sesi="' . $key['sesi'] . '" href="#">Edit</a>';
        $data .= '<a class="dropdown-item btnDelet" data-id="' . $key['id'] . '" href="#">Delete</a>';
        $data .= '</div></div>';
        $data .= '<div class="group-btn">';
        $data .= '<a class="btn btn-xs btn-info btnLinkWp" data-id="' . $key['id'] . '" href=""><i class="mdi mdi-wordpress"> Link</i></a>';
        $data .= ' <a class="btn btn-xs btn-success btnWA" data-id="' . $key['id'] . '" href=""><i class="mdi mdi-whatsapp"> WhatsApp</i></a>';
        $data .= '<a class="btn btn-xs btn-light btnCopy" data-id="' . $key['id'] . '" href=""><i class="mdi mdi-content-copy"> Copy</i></a>';
        $data .= '<a class="btn btn-xs btn-primary" href="' . base_url('master/qrDownload/' . $key['id']) . '"><i class="mdi mdi-qrcode"> Qrcode</i></a>';
        $data .= '<a class="btn btn-xs btn-info btnEdit" data-id="' . $key['id'] . '" data-nama="' . $key['nama'] . '" data-alamat="' . $key['alamat'] . '" data-keterangan="' . $key['keterangan'] . '" data-vip="' . $key['vip'] . '" data-sesi="' . $key['sesi'] . '" href=""><i class="mdi mdi-pencil"> Edit</i></a>';
        $data .= '<a class="btn btn-xs btn-danger btnDelet" data-id="' . $key['id'] . '" href=""><i class="mdi mdi-delete"> Delete</i></a>';
        $data .= '</div>';
        $data .= '</div></div>';
  ...
  {
    $event = $this->session->userdata('sessionEventCek');
    $jml = $this->db->get_where('tamu', ['event_id' => $event])->num_rows();
  ...
  public function newData()
  {
    ...
    if ($cekTamu) {
      $json['kode'] = 3;
      $json['text'] = 'NAMA yang sama sudah terdaftar, Silahkan gunakan NAMA LAIN atau tambahkan Huruf/Karakter';
      ...
      if ($this->db->affected_rows($insert) >= 1) {
        $json['kode'] = 1;
        $json['text'] = 'Berhasil Menyimpan Data';
      } else {
        $json['kode'] = 2;
        $json['text'] = 'Gagal Menyimpan Data, Silahkan Coba Kembali!';
      }
      echo json_encode($json);
    }
  }
  ...
  public function editData()
  {
    ...
    $keterangan = trim($this->input->post('keterangan1', true));
    $vip = $this->input->post('vip1', true);
    $sesi = $this->input->post('sesi1', true);
    ...
    if ($insert) {
      $json['kode'] = 1;
      $json['text'] = 'Update Data Berhasil';
    } else {
      $json['kode'] = 2;
      $json['text'] = 'Gagal Update Data, Silahkan Coba Kembali!';
    }
    echo json_encode($json);
  }
...
public function cekDataWp($id)
  {
    ...
    if ($event['sesi'] > 1) {
      # code...
      $link = $event['urll'] . '?to=' . $nama . '&sesi=Sesi+' . $tamu['sesi'];
    } else {
      # code...
  ...
  public function shareWp($id)
  {
    ...
    if ($event['sesi'] > 1) {
      # code...
      $link = $event['urll'] . '?to=' . $namakode . '&sesi=Sesi+' . $tamu['sesi'];
    } else {
      # code...
  ...
  public function import()
  {
    ...
    if (!$this->db->field_exists('sesi', 'tamu_import')) {
      $this->m_table->addSesiTamuImport();
    }
  ...
  public function pdf()
  {
    ...
    foreach ($id as $key) {
      ...
      $event = $this->m_event->byId($this->session->userdata('sessionEventCek'));
      if (!file_exists('guestbook/assets/images/qr/' . $tamu['qr'] . '.png')) {
        $this->creatBarcode($tamu['nama'], $tamu['qr']);
      }
      array_push($datan, [
        'tamu' => $tamu['nama'],
        'qr' => $tamu['qr'],
    ...
    $data['active'] = $event['sesi'];
    $this->load->view('master/pdf', $data);

    $html = ob_get_contents();
    ob_end_clean();

    require_once('./guestbook/assets/html2pdf/html2pdf.class.php');
    $pdf = new HTML2PDF('P', 'A4', 'fr', true, 'UTF-8', array(20, 10, 20, 10));
    $pdf->setDefaultFont('lato');
    $pdf->WriteHTML($html);
    $pdf->Output('./assets/file/print.pdf', 'V');
  }
