...
class Setting extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $this->load->view('setting/index2', $data);
  }
  ...
  public function editData()
  {
    ...
    $sesi = trim($this->input->post('sesi', true));
    if ($sesi <= 1) {
      $sesi = 1;
    }

    $event = $this->m_event->byId($id);
    if ($event['sesi'] != $sesi && $event['sesi'] > 1) {
      $this->db->set('sesi', 0)->where('event_id', $id)->update('tamu');
    }

    $cekSesi = $this->db->get_where('event_sesi', ['event_id' => $id])->num_rows();
    if ($cekSesi <= 0) {
      for ($i = 0; $i < $sesi; $i++) {
        $data = [
          'event_id' => $id,
        ...
        $this->db->insert('event_sesi', $data);
      }
    } elseif ($cekSesi > 0 && $cekSesi < $sesi || $cekSesi > $sesi) {
      $this->db->where(['event_id' => $id])->delete('event_sesi');
      for ($i = 0; $i < $sesi; $i++) {
        $data = [
          'event_id' => $id,
          'sesi' => $i + 1,
  ...
  public function editData2()
  {
    ...
    $url = htmlspecialchars(trim($this->input->post('urll', true)));
